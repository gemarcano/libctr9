...
#include <ctr9/fatfs/ff.h>			/* Declarations of FatFs API */
#include <ctr9/fatfs/diskio.h>		/* Declarations of disk I/O functions */
...
)
{
	...
	if (!(i % (SS(dp->fs) / SZ_DIRE))) {	/* Sector changed? */
		...
		if (!dp->clust) {		/* Static table */
			...
			if (((i / (SS(dp->fs) / SZ_DIRE)) & (dp->fs->csize - 1u)) == 0) {	/* Cluster changed? */
...
)
{
	...
	i = ((UINT)(dir[LDIR_Ord] & ~LLEF) - 1u) * 13u;	/* Get offset in the LFN buffer */
	s = 0; wc = 1;
	do {
...
)
{
	...
	i = ((UINT)(dir[LDIR_Ord] & 0x3Fu) - 1u) * 13u;	/* Offset in the LFN buffer */
