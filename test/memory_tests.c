...
static bool memory_test1(void *ctx)
{
	...
	data->mem_io = ctr_memory_interface_initialize(data->data, sizeof(data->data));

	for (size_t i = 0; i < sizeof(data->data); ++i)
		data->data[i] = i;

	return data->mem_io != NULL;
}
...
static bool memory_test2(void *ctx)
{
	...
	ctr_io_read_sector(&data->mem_io, buffer, buffer_size, 55, 55);
...
static bool memory_test3(void *ctx)
{
	...
	char *buffer = data->buffer;
	size_t buffer_size = data->buffer_size;
	ctr_io_read(&data->mem_io, buffer, buffer_size, 55, 55);
...
static bool memory_test4(void *ctx)
{
	...
	ctr_io_write_sector(&data->mem_io, stuff, sizeof(stuff), 55);

	return (!memcmp((char*)data->buffer + 55, stuff, sizeof(stuff)));
}
...
static bool memory_test5(void *ctx)
{
	...
	ctr_io_write(&data->mem_io, stuff, sizeof(stuff), 55);
